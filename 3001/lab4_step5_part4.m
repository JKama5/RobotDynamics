...
for i=2:size(points,1)
    for point=1:4
        ...
        TP.plotCubicTrajWithCoefficents(0,travelTime,0,0,0,0,points(i-1,point),points(i,point))
    end
...
% magTSV = [];
% sliceTSV = [];
% for i=1:size(TaskSpaceVel,2)
%     i
%     disp("out of: ");
%     disp(size(TaskSpaceVel,2))
%     sliceTSV = (TaskSpaceVel(1:3, i));
%     magTSV(i) = norm(sliceTSV);
% end
% 
% subplot(1,3,1);
% plot(Time,magTSV(:,:));
% legend('Linear Velocity Magnitude')
% title('Linear Velocity Magnitude vs Time')
% xlabel('Time (s)')
% ylabel('Magnitude of Linear Velocity (mm/s)')
